...
def add_comment_get():
    token = str(services.get_token_by_admin_email(current_user.id))
    logger.info(str(token))
    data = {"username": "Lexi", "comment_object_id": "ede", "comment_text": "wefwe", "token": token}
    data_json = json.dumps(data)
    newHeaders = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    r = request_other.post("http://172.30.0.4/add.comment", json=data_json, headers=newHeaders)
    return "Okey"
    ...
    if request.is_json:
        ...
        if (("username" and "comment_object_id" and "comment_text" and "token") in json_dict) and len(json_dict) == 4:
            ...
            if True:
                ...
                logger.info(username + comment_text + comment_object_id)
